...
assignment:
    T_ID T_SOPEN expression T_SCLOSE T_ASSIGN expression T_SEMICOLON
    {
        ...
        if($3->expr_type != integer || $6->expr_type != integer || symbol_table[*$1].decl_type != array_type )
        {
           std::stringstream ss;
           ss << d_loc__.first_line << ": Type error." << std::endl;
           error( ss.str().c_str() );
...
read:
    T_READ T_OPEN T_ID T_SOPEN expression T_SCLOSE T_CLOSE T_SEMICOLON
    {
        ...
        if(symbol_table[*$5].decl_type != integer || symbol_table[*$3].decl_type != array_type )
        {
           std::stringstream ss;
           ss << d_loc__.first_line << ": Type error." << std::endl;
           error( ss.str().c_str() );
...
|
    T_ID T_SOPEN expression T_SCLOSE
    {
        ...
        $$ = new expression_descriptor(integer,"" +
                                               $3->expr_code + "\n"+
                                               "mov eax, dword["+ symbol_table[*$1].label  + "]+" + "eax" +"]\n"
                                               );
         delete $3;       
    }
|
