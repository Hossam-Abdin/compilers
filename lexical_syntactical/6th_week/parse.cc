// Generated by Bisonc++ V5.02.00 on Sun, 18 Oct 2020 14:48:31 +0200
...
PI__ const s_productionInfo[] = 
{
     ...
     {300, 3}, // 6: declaration (T_TIME_TYPE) ->  T_TIME_TYPE T_ID T_SEMICOLON
     {300, 7}, // 7: declaration (T_INTEGER) ->  T_INTEGER T_ARRAY T_ID T_BOO expression T_BCC T_SEMICOLON
     {299, 1}, // 8: statements ->  statement
     {299, 2}, // 9: statements ->  statement statements
     {302, 2}, // 10: statement (T_SKIP) ->  T_SKIP T_SEMICOLON
     {302, 1}, // 11: statement ->  assignment
     {302, 1}, // 12: statement ->  read
     {302, 1}, // 13: statement ->  write
     {302, 1}, // 14: statement ->  branch
     {302, 1}, // 15: statement ->  loop
     {308, 3}, // 16: num_sem (T_COMMA) ->  expression T_COMMA num_sem
     {308, 1}, // 17: num_sem ->  expression
     {303, 4}, // 18: assignment (T_ID) ->  T_ID T_ASSIGN expression T_SEMICOLON
     {303, 8}, // 19: assignment (T_ID) ->  T_ID T_BOO T_BCC T_EQ T_BO num_sem T_BC T_SEMICOLON
     {303, 7}, // 20: assignment (T_ID) ->  T_ID T_BOO expression T_BCC T_ASSIGN expression T_SEMICOLON
     {304, 5}, // 21: read (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE T_SEMICOLON
     {304, 8}, // 22: read (T_READ) ->  T_READ T_OPEN T_ID T_BOO expression T_BCC T_CLOSE T_SEMICOLON
     {305, 5}, // 23: write (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE T_SEMICOLON
     {306, 5}, // 24: branch (T_IF) ->  T_IF expression T_BEGIN statements T_END
     {306, 9}, // 25: branch (T_IF) ->  T_IF expression T_BEGIN statements T_END T_ELSE T_BEGIN statements T_END
     {307, 5}, // 26: loop (T_WHILE) ->  T_WHILE expression T_BEGIN statements T_END
     {301, 1}, // 27: expression (T_NUM) ->  T_NUM
     {301, 1}, // 28: expression (T_TRUE) ->  T_TRUE
     {301, 1}, // 29: expression (T_FALSE) ->  T_FALSE
     {301, 1}, // 30: expression (T_TIME) ->  T_TIME
     {301, 1}, // 31: expression (T_ID) ->  T_ID
     {301, 3}, // 32: expression (T_ADD) ->  expression T_ADD expression
     {301, 3}, // 33: expression (T_SUB) ->  expression T_SUB expression
     {301, 3}, // 34: expression (T_MUL) ->  expression T_MUL expression
     {301, 3}, // 35: expression (T_DIV) ->  expression T_DIV expression
     {301, 3}, // 36: expression (T_MOD) ->  expression T_MOD expression
     {301, 3}, // 37: expression (T_LESS) ->  expression T_LESS expression
     {301, 3}, // 38: expression (T_GR) ->  expression T_GR expression
     {301, 3}, // 39: expression (T_EQ) ->  expression T_EQ expression
     {301, 3}, // 40: expression (T_AND) ->  expression T_AND expression
     {301, 3}, // 41: expression (T_OR) ->  expression T_OR expression
     {301, 2}, // 42: expression (T_NOT) ->  T_NOT expression
     {301, 3}, // 43: expression (T_OPEN) ->  T_OPEN expression T_CLOSE
     {301, 4}, // 44: expression (T_HOUR) ->  T_HOUR T_OPEN T_ID T_CLOSE
     {301, 4}, // 45: expression (T_MINUTE) ->  T_MINUTE T_OPEN T_ID T_CLOSE
     {301, 4}, // 46: expression (T_ID) ->  T_ID T_BOO expression T_BCC
     {309, 1}, // 47: program_$ ->  program
};
